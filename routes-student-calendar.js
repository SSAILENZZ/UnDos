...
function monthRange(month){const s=new Date(`${month}-01T00:00:00Z`),e=new Date(s);e.setUTCMonth(e.getUTCMonth()+1);return [s.toISOString().slice(0,10),e.toISOString().slice(0,10)]}
r.get('/calendar',async(req,res)=>{try{
  await ensureSchema();const y=await activeYear(),now=new Date(),fallback=`${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,'0')}`,month=validMonth(req.query.month)?String(req.query.month):fallback,[start,end]=monthRange(month);
  ...
  const [subjects,evals,contents]=await Promise.all([
    ...
    pool.query(`SELECT ev.id,ev.name,ev.eval_date::text date,ev.semester,ev.weight::float,ev.status,s.id subject_id,s.name subject_name,u.full_name teacher_name FROM evaluations ev JOIN teaching_assignments ta ON ta.id=ev.assignment_id JOIN subjects s ON s.id=ta.subject_id JOIN users u ON u.id=ta.teacher_id WHERE ta.course_id=$1 AND ta.academic_year_id=$2 AND ta.active=TRUE AND ev.eval_date >= $3 AND ev.eval_date < $4 ORDER BY ev.eval_date,s.name`,[course.course_id,y.id,start,end]),
    pool.query(`SELECT lc.id,lc.content_date::text date,lc.title,lc.description,s.id subject_id,s.name subject_name,u.full_name teacher_name FROM lesson_contents lc JOIN teaching_assignments ta ON ta.id=lc.assignment_id JOIN subjects s ON s.id=ta.subject_id JOIN users u ON u.id=ta.teacher_id WHERE ta.course_id=$1 AND ta.academic_year_id=$2 AND ta.active=TRUE AND lc.content_date >= $3 AND lc.content_date < $4 ORDER BY lc.content_date,s.name`,[course.course_id,y.id,start,end])
  ]);
