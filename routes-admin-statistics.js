...
r.get('/statistics',async(_req,res)=>{
  try{
    ...
    const [base,attendance,courses,subjects,upcoming]=await Promise.all([
      ...
      pool.query(`
        SELECT s.id,s.name,
          COUNT(DISTINCT ta.id)::int assignments,
          COUNT(DISTINCT ev.id)::int evaluations,
          COUNT(DISTINCT ev.id) FILTER(WHERE ev.status='pending')::int pending,
          (SELECT AVG(g.grade)::float FROM grades g JOIN evaluations ev2 ON ev2.id=g.evaluation_id JOIN teaching_assignments ta2 ON ta2.id=ev2.assignment_id WHERE ta2.subject_id=s.id AND ta2.academic_year_id=$1 AND ta2.active=TRUE) grade_average
        FROM subjects s
    ...
    res.json({
      activeYear:y,
      summary:{
        students:Number(b.students||0),teachers:Number(b.teachers||0),courses:Number(b.courses||0),subjects:Number(b.subjects||0),assignments:Number(b.assignments||0),enrollments:Number(b.enrollments||0),
        evaluations:Number(b.evaluations||0),evaluationsCompleted:Number(b.evaluations_completed||0),evaluationsPending:Number(b.evaluations_pending||0),evaluationsNext7:Number(b.evaluations_next7||0),
        gradesCount:Number(b.grades_count||0),gradesAverage:b.grades_average==null?null:Number(b.grades_average),
        attendanceTotal:total,attendancePresent:present,attendanceAbsent:Number(a.absent||0),attendanceDays:Number(a.days||0),attendancePercentage:total?Math.round((present*1000)/total)/10:null
      },
      courses:courses.rows.map(x=>({id:x.id,name:x.name,students:Number(x.students||0),assignments:Number(x.assignments||0),evaluations:Number(x.evaluations||0),gradeAverage:x.grade_average==null?null:Number(x.grade_average),attendanceTotal:Number(x.attendance_total||0),attendancePresent:Number(x.attendance_present||0),attendancePercentage:Number(x.attendance_total||0)?Math.round(Number(x.attendance_present||0)*1000/Number(x.attendance_total))/10:null})),
      subjects:subjects.rows.map(x=>({id:x.id,name:x.name,assignments:Number(x.assignments||0),evaluations:Number(x.evaluations||0),pending:Number(x.pending||0),gradeAverage:x.grade_average==null?null:Number(x.grade_average)})),
      upcoming:upcoming.rows.map(x=>({id:x.id,name:x.name,date:x.date,weight:Number(x.weight),status:x.status,courseId:x.course_id,courseName:x.course_name,subjectId:x.subject_id,subjectName:x.subject_name,teacherName:x.teacher_name}))
    });
