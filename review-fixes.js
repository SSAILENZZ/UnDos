...
function ensureCalendarNav(){const nav=$('#nav');if(!nav||!S.user||S.adminPreview)return;if(S.user.role==='student'&&!nav.querySelector('[data-page="student-calendar"]')){const b=document.createElement('button');b.type='button';b.className='nav-btn';b.dataset.page='student-calendar';b.innerHTML='<span class="nav-icon">▣</span><span>Calendario</span>';const att=nav.querySelector('[data-page="student-attendance"]'),comm=nav.querySelector('[data-page="communications"]'),hist=nav.querySelector('[data-page="student-history"]');if(att)att.after(b);else nav.insertBefore(b,comm||hist||null)}if(S.user.role==='teacher'&&!nav.querySelector('[data-page="teacher-calendar"]')){const b=document.createElement('button');b.type='button';b.className='nav-btn';b.dataset.page='teacher-calendar';b.innerHTML='<span class="nav-icon">▣</span><span>Calendario</span>';const home=nav.querySelector('[data-page="teacher-home"]'),planning=nav.querySelector('[data-page="teacher-planning"]'),comm=nav.querySelector('[data-page="communications"]'),hist=nav.querySelector('[data-page="teacher-history"]');if(home)home.after(b);else nav.insertBefore(b,planning||comm||hist||null)}}
function load(page,label){S.page=page;document.querySelectorAll('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.page===page));const c=$('#content');if(c)c.innerHTML=`<div class="card empty" aria-live="polite">Cargando ${label}…</div>`}
function fail(page,title,e){const c=$('#content');if(!c)return;c.innerHTML=`<div class="card"><h2>${U.esc(title)}</h2><p class="form-error">${U.esc(e?.message||'No se pudo abrir el calendario.')}</p><button type="button" class="btn ghost" id="calendarRetryFinal">Reintentar</button></div>`;const r=$('#calendarRetryFinal');if(r)r.onclick=()=>U.navigate(page)}
const baseNavigate=U.navigate;U.navigate=async page=>{if(page==='student-calendar'){load(page,'calendario');try{if(typeof U.renderStudentCalendar!=='function')throw new Error('El módulo de calendario del estudiante no terminó de cargar.');return await U.renderStudentCalendar(S.studentCalendarMonth||schoolMonth())}catch(e){fail(page,'No se pudo cargar el calendario',e);return}}if(page==='teacher-calendar'){load(page,'calendario docente');try{if(typeof U.renderTeacherCalendar!=='function')throw new Error('El módulo de calendario docente no terminó de cargar.');return await U.renderTeacherCalendar(S.teacherCalendarMonth||schoolMonth())}catch(e){fail(page,'No se pudo cargar el calendario docente',e);return}}return baseNavigate(page)};
const baseShell=U.renderShell;U.renderShell=()=>{baseShell();setTimeout(ensureCalendarNav,0)};
const nav=$('#nav');if(nav)new MutationObserver(()=>setTimeout(ensureCalendarNav,0)).observe(nav,{childList:true});document.addEventListener('click',()=>setTimeout(ensureCalendarNav,0),true);setTimeout(ensureCalendarNav,0);
})();
