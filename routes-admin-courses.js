...
r.delete('/courses/:id',async(req,res)=>{
  try{
    const id=Number(req.params.id),y=await activeYear();
    const q=await pool.query('SELECT id,name FROM courses WHERE id=$1 AND academic_year_id=$2 AND active=TRUE',[id,y.id]);if(!q.rows[0])return apiError(res,404,'Curso no encontrado');
    const deps=await pool.query(`SELECT
    ...
    if(hasData){await pool.query('UPDATE courses SET active=FALSE WHERE id=$1',[id]);return res.json({ok:true,archived:true,message:'El curso fue archivado para conservar su historial académico.'})}
    await pool.query('DELETE FROM courses WHERE id=$1',[id]);res.json({ok:true,archived:false,message:'Curso eliminado.'});
  }catch(e){console.error(e);apiError(res,500,'No se pudo borrar el curso')}
});
module.exports=r;
