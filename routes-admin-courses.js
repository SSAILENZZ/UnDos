...
r.get('/overview',async(_req,res)=>{
  try{
    ...
    const [years,courses,subjects,users,assignments]=await Promise.all([
      ...
      pool.query(`SELECT u.id,u.rut,u.full_name,u.role,u.active,c.id course_id,c.name course_name FROM users u LEFT JOIN enrollments e ON e.student_id=u.id AND e.academic_year_id=$1 AND u.role='student' LEFT JOIN courses c ON c.id=e.course_id ORDER BY CASE u.role WHEN 'admin' THEN 1 WHEN 'teacher' THEN 2 ELSE 3 END,u.full_name`,[y.id]),
      pool.query(`SELECT ta.id,ta.teacher_id,u.full_name teacher_name,ta.subject_id,s.name subject_name,ta.course_id,c.name course_name,ta.active FROM teaching_assignments ta JOIN users u ON u.id=ta.teacher_id JOIN subjects s ON s.id=ta.subject_id JOIN courses c ON c.id=ta.course_id WHERE ta.academic_year_id=$1 AND ta.active=TRUE AND c.active=TRUE AND u.active=TRUE AND s.active=TRUE ORDER BY c.level_order,c.name,s.name,u.full_name`,[y.id])
    ]);
