...
const gradeBucketSql=`CASE WHEN g.grade<3 THEN '2.0–2.9' WHEN g.grade<4 THEN '3.0–3.9' WHEN g.grade<5 THEN '4.0–4.9' WHEN g.grade<6 THEN '5.0–5.9' WHEN g.grade<7 THEN '6.0–6.9' ELSE '7.0' END`;

async function adminDashboard(){
  const y=await activeYear();
  const [summary,monthly,grades,courses,upcoming]=await Promise.all([
    pool.query(`SELECT
      ...
      (SELECT AVG(g.grade)::float FROM grades g JOIN evaluations ev ON ev.id=g.evaluation_id JOIN teaching_assignments ta ON ta.id=ev.assignment_id WHERE ta.academic_year_id=$1 AND ta.active=TRUE) grade_average,
      (SELECT COUNT(*)::int FROM attendance_records ar JOIN teaching_assignments ta ON ta.id=ar.assignment_id WHERE ta.academic_year_id=$1 AND ta.active=TRUE) attendance_total,
      (SELECT COUNT(*)::int FROM attendance_records ar JOIN teaching_assignments ta ON ta.id=ar.assignment_id WHERE ta.academic_year_id=$1 AND ta.active=TRUE AND ar.status='present') attendance_present`,[y.id]),
    pool.query(`SELECT to_char(date_trunc('month',ar.attendance_date),'YYYY-MM') AS "month", COUNT(*)::int AS total, COUNT(*) FILTER(WHERE ar.status='present')::int AS present, COUNT(*) FILTER(WHERE ar.status='absent')::int AS absent FROM attendance_records ar JOIN teaching_assignments ta ON ta.id=ar.assignment_id WHERE ta.academic_year_id=$1 AND ta.active=TRUE GROUP BY 1 ORDER BY 1`,[y.id]),
...
async function teacherDashboard(userId){
  ...
  const [assignments,monthly,grades,upcoming]=await Promise.all([
    ...
    pool.query(`SELECT to_char(date_trunc('month',ar.attendance_date),'YYYY-MM') AS "month", COUNT(*)::int AS total, COUNT(*) FILTER(WHERE ar.status='present')::int AS present, COUNT(*) FILTER(WHERE ar.status='absent')::int AS absent FROM attendance_records ar JOIN teaching_assignments ta ON ta.id=ar.assignment_id WHERE ta.teacher_id=$1 AND ta.academic_year_id=$2 AND ta.active=TRUE GROUP BY 1 ORDER BY 1`,[userId,y.id]),
  ...
  const allGrades=grades.rows.reduce((n,x)=>n+Number(x.total),0);let weighted=0;if(allGrades){const av=await pool.query(`SELECT AVG(g.grade)::float avg FROM grades g JOIN evaluations ev ON ev.id=g.evaluation_id JOIN teaching_assignments ta ON ta.id=ev.assignment_id WHERE ta.teacher_id=$1 AND ta.academic_year_id=$2 AND ta.active=TRUE`,[userId,y.id]);weighted=av.rows[0]?.avg==null?null:round1(av.rows[0].avg)}else weighted=null;
  return {role:'teacher',activeYear:y,summary:{assignments:list.length,students:totals.students,evaluations:totals.evaluations,completed:totals.completed,gradeAverage:weighted,attendancePercentage:attRows.total?pct(attRows.present,attRows.total):null},monthlyAttendance:mapMonthly(monthly.rows),gradeDistribution:distribution(grades.rows),assignments:list,upcoming:upcoming.rows.map(x=>({id:x.id,name:x.name,date:x.date,courseName:x.course_name,subjectName:x.subject_name,weight:Number(x.weight)}))};
}
...
async function studentDashboard(userId){
  ...
  const [subjects,monthly,grades,upcoming,overall]=await Promise.all([
    ...
    pool.query(`SELECT to_char(date_trunc('month',ar.attendance_date),'YYYY-MM') AS "month", COUNT(*)::int AS total, COUNT(*) FILTER(WHERE ar.status='present')::int AS present, COUNT(*) FILTER(WHERE ar.status='absent')::int AS absent FROM attendance_records ar JOIN teaching_assignments ta ON ta.id=ar.assignment_id WHERE ar.student_id=$1 AND ta.academic_year_id=$2 AND ta.course_id=$3 GROUP BY 1 ORDER BY 1`,[userId,y.id,en.id]),
  ...
  return {role:'student',activeYear:y,course:{id:en.id,name:en.name},summary:{subjects:sr.length,gradeAverage:overall.rows[0]?.grade_average==null?null:round1(overall.rows[0].grade_average),grades:Number(overall.rows[0]?.grades||0),attendancePercentage:ar.total?pct(ar.present,ar.total):null,upcoming:upcoming.rows.length},monthlyAttendance:mapMonthly(monthly.rows),gradeDistribution:distribution(grades.rows),subjects:sr,upcoming:upcoming.rows.map(x=>({id:x.id,name:x.name,date:x.date,weight:Number(x.weight),subjectName:x.subject_name,teacherName:x.teacher_name}))};
}

r.get('/',async(req,res)=>{try{if(req.user.role==='admin')return res.json(await adminDashboard());if(req.user.role==='teacher')return res.json(await teacherDashboard(req.user.id));if(req.user.role==='student')return res.json(await studentDashboard(req.user.id));apiError(res,403,'Rol no autorizado')}catch(e){console.error(e);apiError(res,500,'No se pudo cargar el dashboard')}});
module.exports=r;
