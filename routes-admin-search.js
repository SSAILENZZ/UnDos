  ...
  try{
    ...
    const [users,courses,subjects,classes]=await Promise.all([
      pool.query(`
        ...
        FROM users u
        LEFT JOIN enrollments e ON e.student_id=u.id AND e.academic_year_id=$1
      ...
      pool.query(`
        SELECT c.id,c.name,c.active,c.level_order,
          COUNT(DISTINCT e.student_id)::int student_count,
      ...
      pool.query(`
        SELECT ta.id,c.id course_id,c.name course_name,s.id subject_id,s.name subject_name,
          ...
          (SELECT COUNT(*)::int FROM enrollments e WHERE e.course_id=c.id AND e.academic_year_id=$1) student_count,
        ...
        WHERE ta.academic_year_id=$1 AND ta.active=TRUE AND c.active=TRUE
